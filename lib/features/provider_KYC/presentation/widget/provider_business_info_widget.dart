...
class ProviderBusinessInfoWidget extends StatefulWidget {
  final String phoneNumber;
  const ProviderBusinessInfoWidget({super.key, required this.phoneNumber});

  @override
  State<ProviderBusinessInfoWidget> createState() =>
      _ProviderBusinessInfoWidgetState();
}

class _ProviderBusinessInfoWidgetState
    extends State<ProviderBusinessInfoWidget> {
  var loading = false;
      ...
      child: Padding(
        ...
        child: BlocListener<ProviderKycBloc, ProviderKycState>(
          listener: (context, state) {
            if (state is ProviderBusinessInfoKYCSentLoading) {
              setState(() {
                loading = true;
              });
            } else if (state is ProviderBusinessInfoKYCSentSuccess) {
              setState(() {
                loading = false;
              });
              displaySnack(
                  context, "Business info. sent successfully", Colors.black);
            } else if (state is ProviderBusinessInfoKYCSentError) {
              setState(() {
                loading = false;
              });
                      ...
                      onPressed: loading
                          ...
                          : () {
                              if (myKey.currentState!.validate()) {
                                context.read<ProviderKycBloc>().add(ProviderBusinessInfoKYCSent(
